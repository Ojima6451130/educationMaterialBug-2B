...
public class AttendanceLogic {
	...
	public void setEndTime(String employeeId)throws Exception {
		...
		//そのほかのカラムの登録
		calculateAndUpdateActualWorkTime(employeeId, yearMonthDay);
	}
	...
	public void calculateAndUpdateActualWorkTime(String employeeId, String yearMonthDay)throws Exception {
		...
		DateBean today = dateList.stream().filter(d -> d.getYearMonthDay().equals(yearMonthDay)).findFirst().orElse(null);
		...
		boolean isHoliday = today.getPublicHolidayFlg() 
				...
				|| today.getWeekDayEnum() == jp.co.kikin.constant.CommonConstant.DayOfWeek.SUNDAY
				|| dto.isNoShift();
		//拘束時間時間
